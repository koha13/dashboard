<template>
	<div :class="[classes.wrapper]" ref="taggable-select">
		...
		<select v-show="false" multiple class="bordered" :name="name">
			<option v-for="(option, idx) in selectedOptions" :key="idx" value="option">
				{{ option.name }}
			</option>
		</select>
		<div v-if="debug">
					...
					>
						<slot name="pill" v-bind="{ option, idx }" :remove="removeOption">
							<span :class="[classes.pill]">
								<span class="text-sm" v-text="option.name"></span>
	...
	methods: {
		...
		addOption(value) {
			if (value instanceof Event) {
				value = { id: this.negativeId, name: value.target.value };
				this.negativeId--;
			}
